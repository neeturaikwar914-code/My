...
export default function Sidebar({ currentUser, users, chats, selectedChatId, onStartChat, onSelectChat }: SidebarProps) {
    ...
    <aside className="flex h-full min-h-0 flex-col border-r border-slate-200 bg-white md:max-w-sm">
      ...
      <div className="chat-scrollbar min-h-0 flex-1 overflow-y-auto p-2">
        {chatUsers.length ? (
          ...
          <p className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">No chats yet. Start a conversation below.</p>
        )}
